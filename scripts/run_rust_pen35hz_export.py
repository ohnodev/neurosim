#!/usr/bin/env python3
"""
Run 10k ticks at 0.1 ms dt on Rust brain-service with PEN at 35 Hz.
Export replay to world/public for direct comparison with Python 35 Hz run.
"""
...
DT_SEC = DT_MS / 1000.0
DURATION_MS = 1000.0
NUM_STEPS = round(DURATION_MS / DT_MS)
PEN_HZ = 35.0
...
def main() -> int:
    replay_filename = "neurosim_rust_pen35hz_1s_replay.json"
    out_replay = ROOT / "world" / "public" / replay_filename
    scenario = "neurosim_rust_pen35hz_1s"
    ...
    if not pen_ids or not epg_ids:
        print("PEN or EPG set is empty.", file=sys.stderr)
        return 1
